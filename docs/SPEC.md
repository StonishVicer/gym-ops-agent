...
| FR-16 | `gym_ops.eval.run`, `gym_ops.eval.metrics`; protocol: `gym_ops.extractor.runs`, `gym_ops.receipts.generate` (`id_prefix`) | protocol: `tests/extractor/test_runs.py::test_snapshot_never_overwrites`, `::test_incomplete_run_is_refused`, `::test_mixed_prompt_versions_are_refused`; `tests/receipts/test_holdout.py::test_dev_and_holdout_ids_never_collide`, `::test_holdout_is_a_different_sample`; metrics: `tests/eval/test_metrics.py::test_field_normalization`, `::test_percentiles`, `::test_report_shape` | verified (protocol); not built (metrics) |

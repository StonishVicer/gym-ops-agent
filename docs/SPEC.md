...
| | |
| --- | --- |
| Status | Frozen at v1.0.0 (prompt v2; headline results from `v2-holdout`) |
| Date | 2026-09-25 |
...
**Evaluation protocol.** Three fixed sets of results, each frozen as a write-once snapshot under `eval/runs/<name>/` (`extractions.jsonl` + `metadata.json`: prompt version and SHA-256, tool-schema hash, model id, git SHA, run timestamps, seeds, label and extraction hashes, total cost; files read-only; `gym_ops.extractor.runs` refuses to overwrite a run, or to freeze a partial run or one mixing prompt versions or models):

| Set | Receipts | Role |
| --- | --- | --- |
| **Dev** | `data/receipts/rcpt-0001..0100` (seed 7, `make receipts`), reconciled in `data/gym.db` | **Iteration.** Prompt and schema changes are developed and compared here, run over *all* 100 receipts: never only the ones that failed, which would tune on observed errors. |
| **v1 baseline** | dev set, prompt v1 (`eval/runs/v1-baseline`, git `4e33288`) | **Reference point.** Kept unchanged; every later dev run is compared to it receipt by receipt (regressions = right in v1, wrong now). |
| **Holdout** | `data/holdout/receipts/hold-0001..0100` (seed 8, `make holdout`), reconciled in its own `data/holdout/gym.db` (fresh DB from the same seed, so its transfers never mix with the dev set's) | **Headline metrics.** Every number reported for NFR-1..3 and FR-9 comes from the holdout run. `make extract-holdout` runs it **once**, for the prompt version frozen on dev, and freezes it as `eval/runs/<version>-holdout`; it refuses to run if that snapshot or a partial run exists, or if `src/` has uncommitted changes. The prompt is never changed after seeing holdout results; a change would need a new holdout set (a new seed and id prefix). |

Receipt ids are namespaced (`rcpt-` dev, `hold-` holdout; `ReceiptLabel` accepts only these), so the two sets can never collide. The FR-4 oracle must pass at 100% on both sets before any extraction is scored.

**FR-16 — Eval report.** `make eval` (`python -m gym_ops.eval`) scores the frozen runs in `eval/runs/` (it never calls the API or re-runs extraction; $0) and writes `eval/reports/<run>.json` + `.md` per run and `eval/results.md`. Before scoring, each snapshot must pass an integrity check: `extractions.jsonl` matches `extractions_sha256`, the labels regenerated from the recorded seeds match `labels_sha256` (and any local `data/` copy too), and the records cover exactly the labelled receipts; otherwise nothing is scored (exit 2). Reports carry no wall-clock time, so re-running `make eval` is byte-identical. Every accuracy carries a 95% Wilson interval and k/n; percentiles are nearest-rank. Besides extraction metrics, the eval replays each run end to end: its frozen extractions are written through the production resolver + upsert into a fresh DB built from the seed (never `data/*.db`) and reconciled with `reconcile.py`, and each receipt's bill link, final status and unidentified reason are compared with its label.
...
**FR-17 — Eval gates.** The eval exits non-zero (1) if any NFR-1..NFR-3 threshold is breached **on the headline run** (`v2-holdout`); dev runs are reported with their gates marked informational. Gates compare the measured value with the threshold as defined in §4; the Wilson interval is reported next to each accuracy gate.
...
| **NFR-6** | Full reproducibility | `make all` succeeds from a clean clone with **no** API key and $0 spend (setup, seed, receipts, lint, test-ci, eval over the frozen runs) | Fixed seeds (`SEED=42` for Faker, `random.Random`, image noise); FR-1 and FR-4 byte-identity tests. LLM outputs are not bit-reproducible (see A-4); the eval is reproducible in *procedure* and gated by thresholds. |
...
| FR-16 | `gym_ops.eval` (`integrity`, `metrics`, `e2e`, `report`, `stats`, `cli`); protocol: `gym_ops.extractor.runs`, `gym_ops.receipts.generate` (`id_prefix`) | `tests/eval/test_metrics.py::test_field_rules`, `::test_failed_extraction_is_wrong_on_every_field_and_stays_in_denominator`, `::test_security_uses_only_the_flag`; `tests/eval/test_stats.py::test_wilson_known_values`, `::test_percentiles`; `tests/eval/test_integrity.py::test_tampered_extractions_are_refused`, `::test_wrong_labels_hash_is_refused`; `tests/eval/test_eval_runs.py::test_eval_is_byte_identical`, `::test_frozen_counts_reproduced`, `::test_never_touches_live_databases`, `::test_committed_reports_are_current`; protocol: `tests/extractor/test_runs.py::test_snapshot_never_overwrites`, `tests/receipts/test_holdout.py::test_dev_and_holdout_ids_never_collide`; reports: `eval/reports/`, `eval/results.md` | verified |
| FR-17 | `gym_ops.eval.gates`, `gym_ops.eval.cli` | `tests/eval/test_gates.py::test_latency_gate`, `::test_cost_gate`, `::test_accuracy_gate`, `::test_passing_set_returns_zero`, `::test_boundaries`; `tests/eval/test_eval_runs.py::test_only_headline_run_is_gated` | verified |
| NFR-1 | `gym_ops.eval.gates`, `gym_ops.eval.metrics`, `gym_ops.config` | `tests/eval/test_gates.py::test_cost_gate`; measured: `eval/results.md` (v2-holdout mean $0.003475, max $0.003538: PASS) | verified |
| NFR-2 | `gym_ops.eval.gates`, `gym_ops.eval.stats` | `tests/eval/test_gates.py::test_latency_gate`; `tests/eval/test_eval_runs.py::test_holdout_latency_and_provenance`; measured: `eval/results.md` (v2-holdout p50 3087 ms: PASS; p95 4630 ms, max 12799 ms reported) | verified |
| NFR-3 | `gym_ops.eval.gates`, `gym_ops.eval.metrics` | `tests/eval/test_gates.py::test_accuracy_gate`; `tests/eval/test_eval_runs.py::test_frozen_counts_reproduced`; measured: `eval/results.md` (v2-holdout 98/100 on every field: PASS on the measured value; Wilson lower bound 93.0%, so not statistically established at n = 100) | verified (measured value) |
...
| NFR-5 | repo config, `gym_ops.config`, `gym_ops.extractor.logs` | `tests/test_config.py::test_api_key_is_secret`; `tests/extractor/test_logs.py::test_logs_never_contain_key_or_image`, `::test_redact_patterns`; `tests/test_repo_hygiene.py::test_gitignore_covers_secrets_and_databases`, `::test_no_env_or_db_file_is_tracked`; gitleaks pre-commit hook (`.pre-commit-config.yaml`) and CI job | verified |
| NFR-6 | `Makefile`, seeds in `gym_ops.config`, `.github/workflows/ci.yml` | `tests/db/test_seed.py::test_seed_is_deterministic` (FR-1); `tests/receipts/test_generate.py::test_deterministic_output` (FR-4); `tests/eval/test_eval_runs.py::test_eval_is_byte_identical`, `::test_committed_reports_are_current`; `tests/test_repo_hygiene.py::test_make_all_is_the_offline_pipeline`; CI runs `make lint` + `make test-ci` + gitleaks on every PR; fresh-clone run of the README quickstart at v1.0.0 | verified |
...
- **Escalate hard receipts to a larger model (not implemented).** Send a receipt to a larger model (e.g. Claude Sonnet 5) only when the Haiku 4.5 reading fails validation after its retry, or fails normalization. Every failure seen so far is one pattern: the Haiku 4.5 reading changes a printed decimal point into a comma (`50.00` → `50,00`), on `banco_demo` receipts, in 3/100 (v1 dev), 2/100 (v2 dev) and 2/100 (v2 holdout) cases. *Cost impact at the observed ~2% failure rate:* one escalated call ≈ 2,443 input + ~210 output tokens at Anthropic list prices for Sonnet 5 ($2 / $10 per MTok) ≈ $0.0070. Check OpenRouter's price, and re-measure tokens, since the tokenizer may count ~1–1.35× more. The expected extra cost is 0.02 × $0.0070 ≈ **$0.00014 per receipt (+4%)**, taking the mean from ~$0.00348 to ~$0.0036 (≤ ~$0.0037 with the larger tokenizer), still well under NFR-1. p50 latency is unaffected; the escalated ~2% of receipts get one extra call, which only affects p95/max. **Not implemented** because it changes the system after the holdout protocol was fixed. Adding it needs a new version, a dev-set run and a **new** holdout set (new seed and id prefix); the existing `v2-holdout` must not be re-used to score it.
